...
exports.createCustomerPaymentProfile = function(data) {
  ...
  insert = buildPaymentProfileInternals(data.paymentProfile);
  if(!(insert instanceof Array)) response += insert;
  else {
    errors = insert;
    err = true;
...
exports.validateCustomerPaymentProfile = function(data) {
  var response = "", err = false;
  var errors = [];
  ...
  if(data.customerShippingAddressId !== undefined) {
    response += "<customerShippingAddressId>";
    response += data.customerShippingAddressId;
    response += "</customerShippingAddressId>";
  }
  ...
  if(data.validationMode) {
    if(data.validationMode == 'testMode' || data.validationMode == 'liveMode') {
      response += "<validationMode>";
      response += data.validationMode;
      response += "</validationMode>";
    }
